...
- Backend runs on https://localhost:8444.
...
- **ReportedPost**: A user can publish a post that may contain text, image, video or even a
sound clip. Also, a user can edit a post’s caption, if needed.
- **ReportedPost View**: When users click to a post, their views are being collected in order
...
e.g: ```https://localhost:8444/api/files/e52fb3dd-19d0-4e7c-84bd-0f756a962bfb```

*(if the backend server is running on localhost, in port 8444)*
...
```src/main/java/com/example/socialnetworkingapp/model/post/ReportedPostService.java```
