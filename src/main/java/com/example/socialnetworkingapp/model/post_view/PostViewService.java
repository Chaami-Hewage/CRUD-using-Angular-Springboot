...
public class PostViewService {
    ...
    public List<PostViewResponse> getViewsByPostId(Long id) {
        ...
        if(!views.isPresent()) {
            throw new IllegalStateException("ReportedPost with id: " + id.toString() + " does not exist!");
        }
    ...
    public Long getSumOfViewsByPostId(Long id) {
        List<PostView> views = this.postViewRepository.getViewsByPostId(id).orElseThrow(
                () -> new IllegalStateException("ReportedPost with id: " + id.toString() + " does not exist!")
        );
        Long sum = 0L;
